...
- New setting `markdown-links.excludePatterns` to exclude folders/files from the graph using glob patterns. Defaults to `**/node_modules/**` and `**/.git/**` (fixes [#86](https://github.com/tchayen/markdown-links/issues/86)).
...
- Option to configure `markdown-links.wikiLinkAliasDivider` (default to `:`).

### Fixed

- Previous CSP change could prevent graph from loading.
